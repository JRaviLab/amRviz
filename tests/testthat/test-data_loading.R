...
test_that("listAmRmlSpeciesFolders returns empty for NULL/invalid root", {
  ...
  expect_equal(listAmRmlSpeciesFolders("/nonexistent/path"), character(0))
})
...
test_that("listAmRmlSpeciesFolders discovers species dirs in extdata", {
  ...
  # extdata has Shigella_flexneri and Campylobacter_jejuni directories
  expect_true(length(result) > 0)
  # Should be named with directory basenames
...
test_that("listAmRmlSpeciesFolders ignores dirs without perf parquets", {
  ...
  # Create a subdirectory without any parquet files
...
test_that("loadMLResults returns empty tibble when results_root given but no species_dirs", {
  tmp <- tempdir()
  result <- suppressMessages(loadMLResults(results_root = tmp, species_dirs = NULL, verbose = FALSE))
...
test_that("loadMLResults loads demo data in fallback mode", {
  ...
  expect_true("nmcc" %in% names(result) || "bal_acc" %in% names(result))
})
...
test_that("loadMLResults loads from specific species_dirs", {
  ...
  skip_if(length(folders) == 0, "No species dirs with baseline perf in extdata")

  result <- suppressMessages(
    loadMLResults(results_root = extdata, species_dirs = folders[1], verbose = FALSE)
  )
  expect_s3_class(result, "tbl_df")
  expect_gt(nrow(result), 0)
...
test_that("loadTopFeat returns empty tibble when results_root given but no species_dirs", {
  tmp <- tempdir()
  result <- suppressMessages(loadTopFeat(results_root = tmp, species_dirs = NULL, verbose = FALSE))
  expect_s3_class(result, "tbl_df")
  expect_equal(nrow(result), 0)
})
...
test_that("get_metadata_path finds demo metadata parquet", {
  ...
  # Shigella_flexneri has Sfl_metadata.parquet
