...
test_that("queryDataUI returns a shiny tabPanel", {
  ...
  # Should be a tab panel (rendered as <div> with class tab-pane)
...
test_that("quickStatBox accepts custom text_color", {
  result <- quickStatBox("Title", "100", "star", "#fff", text_color = "black")
  expect_s3_class(result, "shiny.tag")
})

test_that("amr_select returns a shiny tag with selectInput", {
  result <- amr_select("test_id", "Test Label", choices = c("A", "B", "C"))
  expect_s3_class(result, "shiny.tag")
})

test_that("amr_select respects multiple parameter", {
  result_multi <- amr_select("id1", "Label", c("A", "B"), multiple = TRUE)
  result_single <- amr_select("id2", "Label", c("A", "B"), multiple = FALSE)
...
test_that("amr_button returns a shiny tag with actionButton", {
  result <- amr_button("btn_id", "Click Me", "download", "btn-primary")
  expect_s3_class(result, "shiny.tag")
})
