...
test_that("makeDatAvailabilityPlot returns a plotly object", {
  df <- tibble::tibble(
    genome_drug.antibiotic = c("ampicillin", "ampicillin", "tetracycline"),
...
test_that("makeIsolationSourcesPlot returns a plotly object", {
  df <- tibble::tibble(
    genome_drug.antibiotic = c("ampicillin", "tetracycline", "ampicillin"),
...
test_that("makeModelPerformancePlot returns empty plotly for NULL data", {
...
test_that("makeModelPerformancePlot returns empty plotly for zero-row data", {
  result <- makeModelPerformancePlot(
...
test_that("makeModelPerformancePlot generates valid plot with demo data", {
  ...
  skip_if(nrow(perf) == 0, "No demo performance data available")
...
test_that("makeNmccStripPlot returns plotly for NULL data", {
  result <- makeNmccStripPlot(NULL)
  expect_s3_class(result, "plotly")
})

test_that("makeNmccStripPlot works with drug class selection", {
  perf <- suppressMessages(loadMLResults(verbose = FALSE))
  skip_if(nrow(perf) == 0, "No demo data available")
  skip_if(
    !"drug_label" %in% names(perf),
    "Demo data missing drug_label column"
  )

  classes <- unique(
    perf$drug_or_class[perf$drug_label == "drug_class"]
  )
  skip_if(
    length(classes) == 0,
    "No drug_class entries in demo data"
  )

  result <- makeNmccStripPlot(
    perf,
    selected_drug_class = classes[1],
    selected_drug = ""
  )
  expect_s3_class(result, "plotly")
})
...
test_that("makeFeatureImportancePlot returns NULL for empty data", {
  result <- makeFeatureImportancePlot(
    ...
    top_n_features = 10, feature_importance_tabset = "across_bug"
  )
  expect_null(result)
})
...
test_that("makeCrossModelPerformancePlot returns NULL for empty data", {
  result <- makeCrossModelPerformancePlot(
    perf_data = NULL, bug = "Sau", drug = "ampicillin", cross_model = "country"
  )
  expect_null(result)
})
...
test_that("makeCrossModelPerformancePlot returns NULL for zero-row data", {
  result <- makeCrossModelPerformancePlot(
    perf_data = data.frame(), bug = "Sau", drug = "ampicillin", cross_model = "country"
  )
  expect_null(result)
})
