...
serverModelPerf <- function(input, output, session, core, results_root) {
  ...
  shiny::observeEvent(input$bug_perf_id, {
    data <- core$queryData() |>
      dplyr::filter(
        normalize_species(.data$species) %in%
          normalize_species(input$bug_perf_id)
      ) |>
  ...
  shiny::observeEvent(input$drug_class_perf_id, {
    shiny::req(input$drug_class_perf_id)
    ...
    if (input$drug_class_perf_id != "all") {
      sp_codes <- normalize_species(input$bug_perf_id)
      ...
      if (nrow(meta) && all(c("class_abbr", "drug_abbr") %in% names(meta))) {
        drugs_in_class <- meta |>
          dplyr::filter(.data$class_abbr %in% input$drug_class_perf_id) |>
  ...
  output$mcc_strip_plot <- plotly::renderPlotly({
    makeMCCStripPlot(
      core$queryData(),
      selected_drug_class = input$drug_class_perf_id,
      selected_drug       = input$drug_perf_id
    )
  })
