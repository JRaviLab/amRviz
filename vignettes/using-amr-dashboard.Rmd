...
results/
└── Shigella_flexneri/
    ├── perf.parquet              # baseline model performance
    ├── country_perf.parquet      # country-stratified models
    ├── year_perf.parquet         # year-stratified models
    ├── cross_perf.parquet        # cross-tested models
    ├── top_features.parquet      # top predictive features
    ├── country_top_features.parquet
    ├── year_top_features.parquet
    ├── metadata.parquet             # isolate surveillance metadata
    └── cluster_feature.parquet      # optional COG/annotation lookup
```

Three file types drive the dashboard:

**Performance metrics** (`*_perf.parquet`) has one row per model. Key columns:
...
**Top features** (`*_top_features.parquet`), ranked predictive features per
...
bird's-eye summary of baseline models: a strip plot of MCC by molecular scale
(left) and heatmaps of median MCC by drug class across scale and data encoding
(right). Here, domain- and gene-scale models reach the highest MCC for most
drug classes.
